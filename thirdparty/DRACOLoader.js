...
class DRACOLoader extends Loader {
  ...
  loadBytes(buffer, onLoad, onError) {
    this.parse(buffer, onLoad, onError);
  }
  ...
  parse(buffer, onLoad, onError = () => {}) {
    this.decodeDracoFile(buffer, onLoad, null, null, SRGBColorSpace, onError)
      .catch(onError);
  }
  ...
  decodeDracoFile(
    ...
    return this.decodeGeometry(buffer, taskConfig).then(callback).catch(
      onError,
    );
  }
...
function DRACOWorker() {
      ...
      case "decode":
        ...
        decoderPending.then((module) => {
          ...
          try {
            ...
            const buffers = geometry.attributes.map((attr) =>
              attr.array.buffer
            );
  ...
  function decodeIndex(draco, decoder, dracoGeometry) {
    ...
    const index = new Uint32Array(draco.HEAPF32.buffer, ptr, numIndices)
      .slice();
    draco._free(ptr);
  ...
  function decodeAttribute(
    ...
    const array = new attributeType(draco.HEAPF32.buffer, ptr, numValues)
      .slice();
    draco._free(ptr);
